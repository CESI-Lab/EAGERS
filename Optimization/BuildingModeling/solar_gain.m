...
sg_diffuse = building.VariableStruct.WallArea*weather.DHIWm2;%Diffuse irradiance (W)
...
n = ones(n_s,1);
sg_direct = 0.25*building.VariableStruct.WallArea*weather.DNIWm2.*(max(0,dot(n*[cosd(or),sind(or),0],dir_norm,2)) + max(0,dot(n*[-cosd(or),-sind(or),0],dir_norm,2)) + max(0,dot(n*[-sind(or),cosd(or),0],dir_norm,2)) + max(0,dot(n*[sind(or),-cosd(or),0],dir_norm,2)));
...
sg.Roof = building.VariableStruct.RoofArea*building.VariableStruct.WallAbsorption*weather.DNIWm2.*max(0,dot(n*[0,0,1],dir_norm,2))/1000;%solar gain (heat) through roof
