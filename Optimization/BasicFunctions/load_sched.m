...
hour = round(3600*24*(date-floor(date)))/3600;%round to nearest second
    ...
    if isfield(sched,'Timestamp')
        ...
        while p2<n_s
            ...
            if ~isempty(dd) 
                ...
                if holiday(d) 
                    if isfield(sched,'Holidays')
                        day = 'Holidays';
                    end
                else
                    switch wd
                        case 1 
                            if isfield(sched,'Sunday')% Sunday
                                day = 'Sunday';
                            elseif isfield(sched,'Sun')% Sunday
                                day = 'Sun';
                            end
                        case 7 
                            if isfield(sched,'Saturday')% Saturday
                                day = 'Saturday';
                            elseif isfield(sched,'Sat')% Saturday
                                day = 'Sat';
                            end
                        otherwise
            ...
            if isfield(sched,'seasons') && length(sched.seasons)>1
                %correct for season
            ...
            if isfield(sched,'interpolate') && sched.interpolate(s) && sched.ramp<1e-3 %EnergyPlus type interpolation
                ...
                if iscell(sched.(day))
                    ...
                    sched_2_interp = convert_sched(sched.(day),1e-3);
                end
